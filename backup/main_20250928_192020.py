...
def list_tasks(
    ...
    con = _conn(); c = con.cursor()
    rows = c.execute(sql, tuple(args)).fetchall()     con.close()      items = []     skipped_ids = []     for row in rows:         try:             items.append(_row_to_task(row))         except Exception as exc:             task_id = None             if isinstance(row, sqlite3.Row):                 try:                     task_id = row["id"]                 except Exception:                     pass             skipped_ids.append(task_id)             logger.warning("list_tasks: skipping task id=%s due to invalid data: %s", task_id, exc)     if skipped_ids:         logger.warning("list_tasks: skipped %d task(s) due to invalid data. ids=%s", len(skipped_ids), skipped_ids)     return items
